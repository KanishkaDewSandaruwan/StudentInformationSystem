...
<body style="background: url(img/back/4QElO0.jpg);
	width: 100%;
	background-size: 100%;">
<form style="padding-top: 70px; border-radius: 3em; " class="edit_pagea" action="doctor_pass.php" method="POST">
			...
			<button class="reg_but button1" type="button" onclick="window.location.href='doctor_dashboard.php'" name="sign_sub">Go to Dashboard</button><br><br>
		...
		if(!empty($currentpass)){
			if(!empty($newpass)){
				if(!empty($confpass)){
					...
					$loginsql="SELECT * FROM doctor WHERE password='".md5($currentpass)."'";
					$result=mysqli_query($con,$loginsql) or mysqli_errno();
					$rows=mysqli_fetch_assoc($result);

					$a=$rows['password'];

					$query5="SELECT password FROM doctor WHERE username='".$g."'";
					...
					{
						if($newpass==$confpass){
							$query3="SELECT * FROM doctor WHERE password='$newpass'";
							...
							else
							{
								...
								{
									$query2="UPDATE doctor SET password='".md5($newpass)."' WHERE username='".$g."'";
