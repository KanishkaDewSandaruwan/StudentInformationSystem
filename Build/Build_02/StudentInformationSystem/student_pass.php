...
<body style="background: url(img/back/4QElO0.jpg);
	width: 100%;
	background-size: 100%;">
<form style="padding-top: 70px; border-radius: 3em; " class="edit_pagea" action="student_pass.php" method="POST">
			...
			<button class="reg_but button1" type="button" onclick="window.location.href='student.php'" name="sign_sub">Go to Dashboard</button><br><br>
		...
		if(!empty($currentpass)){
			if(!empty($newpass)){
				if(!empty($confpass)){
					...
					$loginsql="SELECT * FROM student_login WHERE password='".md5($currentpass)."'";
					$result=mysqli_query($con,$loginsql) or mysqli_errno();
					$rows=mysqli_fetch_assoc($result);

					$a=$rows['password'];

					$query5="SELECT password FROM student_login WHERE regnumber='".$g."'";
					...
					{
						if($newpass==$confpass){
							$query3="SELECT * FROM student_login WHERE password='$newpass'";
							...
							else
							{
								...
								{
									$query2="UPDATE student_login SET password='".md5($newpass)."' WHERE regnumber='".$g."'";
